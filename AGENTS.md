...
- **词条长度**：单条提名不超过 **64 个 Unicode 字符**；AI 生成词条按词计数（1 个英文单词 = 1 词、2 个汉字 = 1 词，`aiEntryWords()`），硬上限 **12 个词**（`aiMaxEntryWords`，解析时超长直接丢弃，另保留 64 rune 绝对上限），prompt 引导 3～7 词
...
- **AI 题材与术语**：prompt 硬性要求每小时词条至少 1/3 完全不含 Ingress 元素（日常恶搞/流行梗）；内置 Ingress 术语表约束动作表述必须符合术语真实语义；词条须贴合对应小时的生活场景（严禁时间错位）并关联节假日/节气等特殊日期；长度要求短语化（按词计数：3～7 词为佳、12 词硬上限），反例池含冗长铺陈样本
